...
class ClassificadorUI:
    def __init__(self, root, base_dir):
        ...
        tk.Button(col1, text="Remover Fundo", command=self.remover_fundo).pack(pady=2)
    ...
    def _carregar_logo(self):
        ...
        try:
            ...
            logo_frame.grid(row=2, column=2, sticky="se", padx=10, pady=10)
            tk.Label(logo_frame, image=self.logo_img, bg=BG_COLOR).pack(anchor="se")
        except Exception:
            self.logo_img = None
    ...
    def remover_fundo(self):
        ...
        nova_imagem = ProcessImageFunctions.remover_fundo(self.img)
        if nova_imagem is None:
            return
        self.img = nova_imagem
        self._atualizar_canvas()
    ...
    def separar_componentes(self):
        ...
        salvos = ProcessImageFunctions.separar_componentes(self.img, pasta_destino)
